<template>
  <div class="app-container">
    ...
    <el-row v-if="isMobile" :gutter="10">
      <el-col :xs="24" :sm="12" :md="8" v-for="item in locationList" :key="item.id">
    ...
    <el-table v-else v-loading="loading" :data="locationList" @selection-change="handleSelectionChange" @sort-change="handleSortChange">
...
<script>
import { listEngineeringPatrolLocation, getEngineeringPatrolLocation, delEngineeringPatrolLocation, addEngineeringPatrolLocation, updateEngineeringPatrolLocation, exportEngineeringPatrolLocation } from "@/api/engineering/location/engineeringPatrolLocation";
import QRCode from 'qrcode';

export default {
  name: "EngineeringPatrolLocation",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      listEngineeringPatrolLocation(params).then(response => {
    ...
    handleUpdate(row) {
      ...
      getEngineeringPatrolLocation(id).then(response => {
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.isUpdate()) {
            updateEngineeringPatrolLocation(this.form).then(response => {
              this.$modal.msgSuccess(this.$t('engineeringLocation.editSuccess'));
              this.open = false;
              this.getList();
            });
          } else {
            addEngineeringPatrolLocation(this.form).then(response => {
    ...
    /** 判断是否为修改操作 */
    isUpdate() {
      return this.form.id && this.locationList.some(location => location.id === this.form.id);
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      const ids = row.id || this.ids;
      this.$modal.confirm(this.$t('engineeringLocation.deleteConfirm', { ids })).then(function() {
        return delEngineeringPatrolLocation(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('engineering/patrolLocation/export', {
        ...this.queryParams
      }, `engineering_patrol_location_${new Date().getTime()}.xlsx`)
    },
    ...
    generateLocationId() {
      ...
      const maxId = Math.max(...this.locationList.map(location => parseInt(location.id, 10)));
    ...
    viewQRCode(id) {
      const qrCodeUrl = `https://schoolmaintain-webapp.azurewebsites.net/cn/inspection_page/${id}`;
    ...
    generateCheckInLink(id) {
      return `https://schoolmaintain-webapp.azurewebsites.net/cn/inspection_page/${id}`;
    }
  }
};
</script>
